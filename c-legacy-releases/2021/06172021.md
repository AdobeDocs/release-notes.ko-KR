...
| 기능 | 설명 |
| ------- | ----------- |
| [!UICONTROL 캠페인] | (Google 광고 캠페인) [!UICONTROL Advertising Cloud Search] 이제 쇼핑 캠페인에서 로컬 인벤토리 광고에 대한 속성을 동기화, 최적화 및 제공합니다.<br><br>Google 광고 내에서 로컬 인벤토리 광고를 설정 및 관리합니다. |
|  | 여러 캠페인 및 캠페인 구성 요소를 편집할 때 보류 중 보기에서 검토할 초안으로 변경 사항을 저장하거나 특정 시간에 대한 변경 사항을 예약할 수 없습니다. 기존의 모든 예약된 작업은 예약된 대로 계속 실행됩니다. |
| [!UICONTROL 광고 인사이트] | 다음 인사이트는 베타 모드에서 사용할 수 있습니다.<ul><li>[!UICONTROL 정규화된 Sim(결합된) 베타]</li><li>[!UICONTROL Portfolio Launch 베타]</li> |
| [!UICONTROL 알림 센터 Beta] | 이제 를 설치하여 브라우저 외부에서 알림을 받고 관리할 수 있습니다 [!UICONTROL 알림 센터 베타] Google Chrome 40 이상 또는 Microsoft Edge 17 이상용 애플리케이션 애플리케이션이 일반 브라우저 버전보다 빠르게 로드되며 자동으로 업데이트됩니다.<br><br>응용 프로그램을 설치하면 웹 응용 프로그램은 창 크기에 따라 레이아웃을 동적으로 다시 정렬하는 별도의 창으로 로드됩니다. 애플리케이션에 로그인하거나 브라우저 창을 열어 사용할 필요가 없습니다. |
|  | 새 경고 유형, [!UICONTROL Campaign Management UI 작업]를 사용할 수 있으며 포트폴리오, 제한, 분류 및 이벤트에 대한 일괄 편집 및 할당 변경 등을 포함하여 모든 백그라운드 작업의 최종 상태를 표시합니다. UI 작업에 대한 경고를 자동으로 구독합니다.<br><br>이 경고 유형은 결국 올바른 도구 모음에서 사용할 수 있는 기존 작업 상태 알림 (이전 작업 상태 알림)을 대체합니다. |
| 보고서 | 새로운 [!UICONTROL 캠페인 시간별 보고서] 지정된 날짜 범위에서 각 시간에 대한 키워드가 있는 하나 이상의 Google 캠페인에 대한 비용, 클릭 및 (선택 사항) 전환 데이터를 표시합니다. 이 보고서는 [!UICONTROL 캠페인 보고서] 를 &quot;시간별&quot; 또는 &quot;요일(시간별)&quot;의 데이터 집합으로 사용. 이러한 집계 유형은 [!UICONTROL 캠페인 보고서] 추가 릴리스에서 사용할 수 있습니다. |
